...
def handle_delivery_request(main_service, req_data, client_socket):
    try:
        ...
        user_id, dest, items = req_data.get("user_id"), req_data.get("destination"), req_data.get("items")
        ...
        roscar = main_service.roscars_session.query(RosCars)\
            .filter_by(operational_status=OperationalStatus.STANDBY)\
            .order_by(RosCars.battery_percentage.desc()).first()
        if not roscar:
            raise RuntimeError("사용 가능한 Roscar 없음")
        delivery = Delivery(user_id=user_id, roscar_id=roscar.roscar_id,
                            delivery_status="TO_DO", driving_phase_id=None,
        ...
        main_service.logger_session.add(
            DeliveryEventLog(delivery_id=delivery.delivery_id,
                             user_id=user_id,
                             previous_event="WAIT",
        ...
        first_task = None
        for it in items:
            sid, lid, qty = it.get("shoes_model_id"), it.get("location_id"), it.get("quantity", 0)
            if qty <= 0: continue
            for _ in range(qty):
                t = Task(delivery_id=delivery.delivery_id, shoes_model_id=sid,
                         location_id=lid, status="TO_DO")
                main_service.roscars_session.add(t)
                main_service.roscars_session.flush()
                main_service.logger_session.add(
                    TaskEventLog(task_id=t.task_id,
                                 previous_event="WAIT",
...
def handle_cancel_task(main_service, req_data, client_socket):
    delivery_id, user_id = req_data.get("delivery_id") or 0, req_data.get("user_id")
    try:
